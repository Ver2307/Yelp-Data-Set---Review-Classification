...
def load_data(file_name):
    ...
    print('%s is being loaded.' %file_name)
...
def generate_labels(review_data, threshold): # 0.60
    ...
    print('Distribution over the two classes in the entire data set is:')
...
def merge_review_and_user_data(review_data, user_data):
    ...
    print('----- Merge review and user data -----')

    return review_data.merge(user_data, how='left', on='user_id', suffixes=('_review', '_user'))


def downsample_data(merged_data, share_to_keep):
    """ Downsample the data to speed up computation time"""

    print('----- Downsample data set -----')
    percentage_to_keep = share_to_keep * 100
    print('Share of data being used is %s percent of original data set' % percentage_to_keep)

    return merged_data.sample(frac = share_to_keep, random_state = 101).reset_index()
...
def train_and_evaluate_model(training_x, training_y, test_x, test_y):
    ...
    print('Number of trees: %s' %args.no_trees_rf[0])
    print('Max Depth: %s' % args.max_depth[0])
    print('Max Features: %s' % args.max_features[0])
    model = RandomForestClassifier(n_estimators=args.no_trees_rf[0], max_depth=args.max_depth[0], random_state = 101,
                                   max_features=args.max_features[0], class_weight='balanced',n_jobs = cores)
...
def main(share_to_keep):
    print('----- Load data -----')
    users = load_data('user.json')
    print('This might take a while ...')
    reviews = load_data('review.json')
    generate_labels(reviews, 0.6)
    merged = merge_review_and_user_data(reviews, users)
    del users
    del reviews
    gc.collect()
    if share_to_keep is not None:
        merged = downsample_data(merged, share_to_keep)
...
if __name__ == "__main__":
    ...
    parser.add_argument('share_to_keep', metavar='share', type=float, nargs=1,
    ...
    parser.add_argument('no_trees_rf', metavar='trees', type=int, nargs=1, help='Number of trees for random forest')
    parser.add_argument('max_depth', metavar='max_d', type=int, nargs=1, help='Max Depth for random forest')
    parser.add_argument('max_features', metavar='max_f', type=int, nargs=1, help='Size of random variable subspace for random forest')
